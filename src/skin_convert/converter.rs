...
use crate::common::skin::SkinModel;
use crate::common::skin::SkinModel::{Classic, Slim};
...
fn convert_geometry(
    skin_data: &[u8],
    mut skin_width: usize,
    client_claims: &Value,
    geometry_data: &[u8],
    geometry_patch: &JsonValue,
    geometry_name: &str,
) -> Result<(Vec<u8>, Option<SkinModel>), &'static str> {
    ...
    if animated_face.is_string() {
        ...
        let face_data = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, face_data.unwrap());
        if face_data.is_err() {
...
fn get_correct_entry<'a>(
    format_version: &'a str,
    geometry_data: &'a JsonValue,
    geometry_name: &'a str,
) -> Result<(&'a JsonValue, usize, usize), &'static str> {
...
fn translate_poly_bone(
    ...
    let mut highest_u = 0_f64;
    let mut lowest_v = h_f;
    let mut highest_v = 0_f64;
...
fn bone_name_to_skin_section(bone_name: &primitive::str) -> Option<SkinSection<'_>> {
    match bone_name {
