...
use crate::skin_convert::ErrorType::{InvalidGeometry, InvalidSize, MissingField};
...
pub fn collect_skin_info(client_claims: &Value) -> Result<SkinInfo, ErrorType> {
    let skin_width = client_claims["SkinImageWidth"].as_u64()
        .ok_or(MissingField)? as usize;
    let skin_height = client_claims["SkinImageHeight"].as_u64()
        .ok_or(MissingField)? as usize;

    let skin_data = client_claims["SkinData"].as_str()
        .ok_or(MissingField)?;
    let raw_skin_data = BASE64.decode(skin_data)
        .map_err(|_| MissingField)?;
    ...
    let geometry_patch_res = BASE64.decode(resource_patch_option.unwrap());
...
pub fn encode_custom_image(raw_data: &[u8], width: usize, height: usize) -> ImageWithHashes {
    ...
    encoder.info_png_mut().interlace_method = 0;
    ...
    ImageWithHashes {
        ...
        minecraft_hash: Box::from(minecraft_hash.as_ref()),
        hash: Box::from(hash.as_ref()),
    }
}
